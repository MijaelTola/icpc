...
int main(){
    ...
    int n,m,k;
    cin >> n >> m >> k;
    MinCostFlow flow(n + m + 3);
    flow.addEdge(n + m + 1,n + m + 2,0,k);

    for (int i = 1; i < n + 1; ++i) {
        int a,p,b,q;
        cin >> a >> p >> b >> q;
        flow.addEdge(0,i,0,1);
        flow.addEdge(i,a + n,p,10000);
        flow.addEdge(i,b + n,q,10000);
    }
    
    for (int i = n + 1; i < n + 1 + m; ++i) {
        flow.addEdge(i, n + m + 1,0,10000);
    }
    pair<int,int> ans = flow.solve(0,n + m + 2);
